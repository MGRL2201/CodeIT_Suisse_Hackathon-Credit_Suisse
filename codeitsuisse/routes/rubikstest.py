...
@app.route("/rubikstest", methods=["GET", "POST"])
def rubikstest():
    data = request.get_json()
    inputValue1 = data.get("ops1")
    inputValue2 = data.get("state1")
    inputValue3 = data.get("ops2")
    inputValue4 = data.get("state2")
    result = test(inputValue1, inputValue2, inputValue3, inputValue4)
...
def test(ops1, state1, ops2, state2):
    return make_moves(ops1, state1) == make_moves(ops2, state2)
